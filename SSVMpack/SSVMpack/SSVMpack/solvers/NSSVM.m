...
if  n  <  3e4 
    ...
    Qt = spdiags(y,0,m,m)*A; 
end
 
Q     = Qt';   
Fnorm = @(var)norm(var,'fro')^2;

[maxit,alpha,tune,disp,tol,eta,s0,C,c] = GetParameters(m,n,pars); 
...
%Initial parameters --------------------------------------------------------
function [maxit,alpha,tune,disp,tol,eta,s0,C,c] = GetParameters(m,n,pars)
maxit   = 1e3;
alpha   = zeros(m,1);
tune    = 0; 
disp    = 1;
