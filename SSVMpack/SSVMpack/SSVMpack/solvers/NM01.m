function out = NM01(A,y,pars)
% This code aims at solving the support vector machine with form
%
%      min  ||Dx||^2 + lam * sum_{i=1}^m l_{0/1}(1-y_i<a_i,x>)
%
% where D is a diagonal matrix with D_{11}=...=D_{n-1,n-1}=1, D_{nn}=w>0
% lam > 0 
% l_{0/1}(t) = 1 if t > 0 and 0 if t <= 0
% y_i \in\{1,-1\} is the i-th entry of y
% a_i = (a0_i;1) \in \R^n with a0_i being the i-th row of A
% =========================================================================
% Inputs:
%     A   : The sample data in \R^{m-by-(n-1)}   (required)
%     y   : The classes of the sample in \R^m    (required)
%           y_i \in {+1,-1}, i=1,2,...,m             
...
if  n   < 1e4
    ...
    Any = [spdiags(ny,0,m,m)*A ny];    
end
Fnorm   = @(var)norm(var,'fro')^2;
n       = n+1; 
[maxit,tau,w,mu,disp,acc0,x,cgtol] = GetParameters(m,n,pars);
...
for iter     = 1:maxit
    ...
    stop3 = (raw < 1e-8 && acc>0.5 && iter>4); 
    if stop1  || ( acc>= acc0-1e-5) || stop2 || stop3
       break;
    end   
...
%--------------------------------------------------------------------------
function [maxit,tau,w,mu,disp,acc0, x0,tolcg] = GetParameters(m,n,pars)
maxit = 1e3;
w     = 1e-8;
mn    = max(m,n);
disp  = 1;
...
mu    = 0.001*(mn<500) + ...
        ...
        10*(mn>=500 & m>n); 
x0    = zeros(n,1); 
if isfield(pars,'acc');    acc0  = pars.acc;     end
