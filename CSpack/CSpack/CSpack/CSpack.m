...
if ~isfield(pars,'lambda')
    if  isa(A,'function_handle')
        lambda  = 0.005*norm(At(b),'inf');
    else
        lambda  = 0.005*norm(b'*A,'inf');
    end
    lambda_NL0R = 10*lambda;
else
    lambda      = pars.lambda;
    lambda_NL0R = lambda;
end
...
if ismember(solver,{'NL0R','PSNP','MIRL1'})
    thresh = 0;  
    if  (out.error > 1e-2 && ~isa(A,'function_handle')) || ~isempty(s)
        if  isempty(s)
            sx      = sort(abs(out.sol(out.sol~=0))); 
            [~,it]  = max(normalize(sx(2:end)./sx(1:end-1)));
            if ~isempty(it); thresh = sx(it(1)); end 
        else
    ...
    if ~isa(A,'function_handle')
        Ts      = find(out.sol~=0); 
        xs      = zeros(n,1);            
        xs(Ts)  = linsolve(A(:,Ts),b); 
        out.sol = xs;  
        out.obj = norm(A(:,Ts)*xs(Ts)-b)^2/2;   
        
        T       = find(abs(out.sol)>thresh); 
        x       = zeros(n,1);            
        x(T)    = linsolve(A(:,T),b);   
        obj     = norm(A(:,T)*x(T)-b)^2/2; 
        if obj  < out.obj || abs(length(T)-length(Ts)) < 0.75*length(Ts)
            out.sol = x; 
            out.obj = obj; 
        end
    end
